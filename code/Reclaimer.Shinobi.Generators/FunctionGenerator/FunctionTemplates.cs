...
{{ if struct.has_ctor }}using Reclaimer.Shinobi.Memory;{{ end }}
    ...
    public unsafe static partial class Resolver {
        private static void InitializeMemberFunctions(SigScanner s)
        {
            ...
            try {
                ...
                Log.Warning($""[Reclaimer.Shinobi] function {{ struct.name }}::{{ mf.name }} failed to match signature {{ mf.signature }} and is unavailable"");
            }
            {{~ end ~}}
            {{~ end ~}}
        }
